...
double t;
//double R = 3479;					//抵抗	R[Ω]

//double L = 51.4e-4;					//インダクタンス L[H]
...
void main() {
	...
	sprintf_s(moji, "電圧の時間変化(0.3Mhz).csv", count);
	fopen_s(&fp, moji, "w");

	sprintf_s(moji, "電流の時間変化(0.3Mhz).csv", count);
	fopen_s(&fp1, moji, "w");

	sprintf_s(moji, "抵抗の場所の電圧(0.3Mhz).csv", count);
	fopen_s(&fp2, moji, "w");

	sprintf_s(moji, "抵抗の場所の電圧((0.3Mhz).txt", count);
	fopen_s(&fp3, moji, "w");

	sprintf_s(moji, "電流の時間変化((0.3Mhz).txt", count);
	fopen_s(&fp4, moji, "w");

	sprintf_s(moji, "磁界の時間変化((0.3Mhz).txt", count);
	fopen_s(&fp5, moji, "w");
	...
	for (t = 0; t <= 1.0e-6; ) {
		...
		if (t > 1.0e-6 && t <= 0.059e-7) {
		double R = 3479;
		double L = 5.14e-3;

		if (t > 0.059e-7 && t <= 0.01e-7) {
		double R = 1177;
		double L = 5.97e-3;
		*/

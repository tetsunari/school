...
void main() {
	...
	sprintf_s(moji, "電圧の時間変化(確認).csv", count);
	fopen_s(&fp, moji, "w");

	sprintf_s(moji, "電流の時間変化(確認).csv", count);
	fopen_s(&fp1, moji, "w");

	sprintf_s(moji, "抵抗の場所の電圧(確認).csv", count);
	fopen_s(&fp2, moji, "w");

	sprintf_s(moji, "抵抗の場所の電圧(確認).txt", count);
	fopen_s(&fp3, moji, "w");

	sprintf_s(moji, "電流の時間変化(確認).txt", count);
	fopen_s(&fp4, moji, "w");

	sprintf_s(moji, "磁界の時間変化(確認).txt", count);
	fopen_s(&fp5, moji, "w");
	...
	for (t = 0; t <= 1.0e-6; ) {
		...
		if (t <= 1.6e-7) {
			Ez[x_s][y_s][z_v] = (t * (2.8 / 1.6e-7)) / dz;
			V = Ez[x_s][y_s][z_v] * dz;
		}
		else {
			Ez[x_s][y_s][z_v] = 2.8 / dz;
